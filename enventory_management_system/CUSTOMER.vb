Imports MySql.Data.MySqlClient

Public Class CUSTOMER
    Dim connString As String = "server=127.0.0.1;userid=root;password='';database=inventory_system"
    Dim conns As New MySqlConnection(connString)

    Public Sub populate()

        conns.Open()
        Dim sql = "select * from customer_table"
        Dim adaptor As MySqlDataAdapter
        adaptor = New MySqlDataAdapter(sql, conns)
        Dim builder As MySqlCommandBuilder
        builder = New MySqlCommandBuilder(adaptor)
        Dim ds As DataSet
        ds = New DataSet
        adaptor.Fill(ds)
        customer_data_view.DataSource = ds.Tables(0)
        conns.Close()

    End Sub
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click

        Try
            conns.Open()
            MessageBox.Show("Connection to MySQL test database was successful!!!!", "TESTING      CONNECTION TO MySQL DATABASE")
            Dim query As String
            query = "insert into customer_table values (" & customer_id.Text & ",'" & customer_name.Text & "' , '" & customer_phone.Text & "')"
            Dim cmd As MySqlCommand
            cmd = New MySqlCommand(query, conns)
            cmd.ExecuteNonQuery()
            MsgBox("customer_added_sucessfully.")
            conns.Close()
        Catch ex As Exception
            MsgBox(ex.Message)
            conns.Close()

        End Try
        populate()
    End Sub

    Private Sub CUSTOMER_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        populate()
    End Sub

    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click

        Dim command As New MySqlCommand("DELETE FROM `customer_table` WHERE `customer_id` = @id", conns)

        command.Parameters.Add("@id", MySqlDbType.Int64).Value = customer_id.Text

        conns.Open()

        Try

            If command.ExecuteNonQuery() = 1 Then

                MessageBox.Show("the customer has been removed ")

            Else

                MessageBox.Show("Error")

            End If

        Catch ex As Exception

            MessageBox.Show("Something Wrong")

        End Try

        conns.Close()
        populate()

    End Sub

    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        customer_id.Text = ""
        customer_name.Text = ""
        customer_phone.Text = ""
    End Sub

    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        If customer_id.Text = "" Or customer_name.Text = "" Or customer_phone.Text = "" Then
            MsgBox("incomplete data.")
        Else
            conns.Open()
            Dim sql = "update customer_table set customer_name='" & customer_name.Text & "' , customer_phone='" & customer_phone.Text & "' where customer_id = " & customer_id.Text & " "
            Dim cmd As New MySqlCommand(sql, conns)
            cmd.ExecuteNonQuery()
            MsgBox("the catagory has been edited")
            conns.Close()
            populate()
        End If

    End Sub

    Private Sub OrderToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles OrderToolStripMenuItem.Click
        ORDER.Show()
        Me.Close()

    End Sub

    Private Sub CatagoryToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles CatagoryToolStripMenuItem.Click
        CATAGORY.Show()
        Me.Close()

    End Sub

    Private Sub LogoutToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles LogoutToolStripMenuItem.Click
        USER.Show()
        Me.Close()

    End Sub

    Private Sub OrderToolStripMenuItem1_Click(sender As Object, e As EventArgs) Handles OrderToolStripMenuItem1.Click
        ORDER.Show()
        Me.Close()
    End Sub

    Private Sub ProductToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ProductToolStripMenuItem.Click
        PRODUCT.Show()
        Me.Close()

    End Sub

    Private Sub LogoutToolStripMenuItem1_Click(sender As Object, e As EventArgs) Handles LogoutToolStripMenuItem1.Click
        Form1.Close()
        Me.Close()

    End Sub

    Private Sub RefreshToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles RefreshToolStripMenuItem.Click
        populate()

    End Sub

    Private Sub BynameToolStripMenuItem2_Click(sender As Object, e As EventArgs) Handles BynameToolStripMenuItem2.Click
        customer_search_byname()
    End Sub

    Private Sub ByidnoToolStripMenuItem1_Click(sender As Object, e As EventArgs) Handles ByidnoToolStripMenuItem1.Click
        customer_search_byid()

    End Sub
    Public Sub customer_search_byid()
        Dim id As String
        id = InputBox("PLEASE ENTER THE CUSTOMER ID ")

        Dim connString As String = "server=127.0.0.1;userid=root;password='';database=inventory_system"

        Dim conns As New MySqlConnection(connString)
        conns.Open()
        Dim sql = "select * from customer_table where customer_id like '%" & id & "%'"
        Dim adaptor As MySqlDataAdapter
        adaptor = New MySqlDataAdapter(sql, conns)
        Dim builder As MySqlCommandBuilder
        builder = New MySqlCommandBuilder(adaptor)
        Dim ds As DataSet
        ds = New DataSet
        adaptor.Fill(ds)
        customer_data_view.DataSource = ds.Tables(0)
        conns.Close()
    End Sub
    Public Sub customer_search_byname()
        Dim id As String
        id = InputBox("PLEASE ENTER THE CUSTOMER NAME ")

        Dim connString As String = "server=127.0.0.1;userid=root;password='';database=inventory_system"

        Dim conns As New MySqlConnection(connString)
        conns.Open()
        Dim sql = "select * from customer_table where customer_name like '%" & id & "%'"
        Dim adaptor As MySqlDataAdapter
        adaptor = New MySqlDataAdapter(sql, conns)
        Dim builder As MySqlCommandBuilder
        builder = New MySqlCommandBuilder(adaptor)
        Dim ds As DataSet
        ds = New DataSet
        adaptor.Fill(ds)
        customer_data_view.DataSource = ds.Tables(0)
        conns.Close()
    End Sub

    Private Sub HomepageToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles HomepageToolStripMenuItem.Click
        HOME_PAGE.Show()
        Me.Hide()

    End Sub
End Class